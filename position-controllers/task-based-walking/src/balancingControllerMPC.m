...
function [Hessian,gradient,ConstraintMatrix_equality,biasVectorConstraint_equality, ...
    ...
    % Separate the rotational task from the feet and CoM tasks
    JConstraints = J(1:15, :);
    JRotTask     = J(16:18, :);
    
    JDotNuConstraints = JDot_nu(1:15, :);
    ...
    acc_constraints_star = acc_task_star(1:15);
    ...
    Hessian   =  transpose(St_invM_B)*St_invM_B + Sat.weight_tau*transpose(S_tau)*S_tau + ...
                 ...
                 Sat.weight_rotationalTask*transpose(JRotTask_invM_B)*JRotTask_invM_B;
             
    gradient  = -transpose(St_invM_B)*(St_invM_h +sDDot_star) -transpose(JRotTask_invM_B)*(acc_rotTask_star - JDotNuRotTask + JRotTask_invM_h);

    % Multiplier of u in the equality constraint equation and bias terms
    JConstraints_invM_B  = JConstraints*invM*B;
    JConstraints_invM_h  = JConstraints*invM*h;
    
    % Equality constraints
    ConstraintMatrix_equality     = JConstraints_invM_B;
    biasVectorConstraint_equality = acc_constraints_star -JDotNuConstraints +JConstraints_invM_h;
