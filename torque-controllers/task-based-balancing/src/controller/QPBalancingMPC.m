...
function QPBalancingMPC(block)
    ...
    function Outputs(block)
        ...
        % errors, a small tolerance is added to the bias vectors  Note that equality constraints are
        % expressed in the form of: b_eq < A*x < b_eq
        eps      = 0.0001*ones(length(biasVectorConstraint_equality),1);
        ...
        if feetInContact(1) > (1-Sat.toll_feetInContact) && feetInContact(2) < Sat.toll_feetInContact
            ...
            if Config.feetInCostWhenSwing
                prunedConstraintMatrix_equality = ConstraintMatrix_equality([1:6,13:end],[1:6,13:end]); 
                prunedBiasVectorConstraint_equality = biasVectorConstraint_equality([1:6,13:end]);
            else
                prunedConstraintMatrix_equality = ConstraintMatrix_equality(:,[1:6,13:end]);
                prunedBiasVectorConstraint_equality = biasVectorConstraint_equality;
            end
            eps      = 0.0001*ones(length(prunedBiasVectorConstraint_equality),1);
             
            A    = [ConstraintMatrix_inequality(:,1:6), zeros(length(biasVectorConstraint_inequality),ROBOT_DOF);
                prunedConstraintMatrix_equality];
            
            ubA  = [biasVectorConstraint_inequality;
                (prunedBiasVectorConstraint_equality+eps)];
        ...
        elseif feetInContact(1) < Sat.toll_feetInContact && feetInContact(2) > (1-Sat.toll_feetInContact)
            ...
            % foot contact only.
            if Config.feetInCostWhenSwing
                prunedConstraintMatrix_equality = ConstraintMatrix_equality(7:end,7:end);
                prunedBiasVectorConstraint_equality = biasVectorConstraint_equality(7:end);
            else
                prunedConstraintMatrix_equality = ConstraintMatrix_equality(:,7:end);               
                prunedBiasVectorConstraint_equality = biasVectorConstraint_equality;
            end
            eps      = 0.0001*ones(length(prunedBiasVectorConstraint_equality),1);
