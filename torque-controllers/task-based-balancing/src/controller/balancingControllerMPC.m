...
%                              w_H_l_sole, w_H_r_sole, Sat, weightsForces, leftIsFixed)      
%
...
%        - weightsForces = [2 * 2] weights for minimizing contact forces in the cost function
%        - leftIsFixed = boolean to verify if the left foot is on gorund
%
...
function [Hessian,gradient,ConstraintMatrix_equality,biasVectorConstraint_equality, ...
          ConstraintMatrix_inequality,biasVectorConstraint_inequality, sDDot_star] = ...
          balancingControllerMPC(feetInContact, M, h, J, impedances, ...
                                 dampings, s, sDot, s_sDot_sDDot_des, acc_task_star, ...
                                 JDot_nu, ConstraintsMatrix_feet, biasVectorConstraint_feet, ...
                                 w_H_l_sole, w_H_r_sole, Sat, Config, weightsForces, leftIsFixed) 
                           
    persistent previousContactState initialLeftIsFixed
    
    if isempty(previousContactState)
        previousContactState = feetInContact;
    end
    if isempty(initialLeftIsFixed)
        initialLeftIsFixed = ~leftIsFixed; % for having the weight on forces at startup
    end
...
%                  -diag(dampings)*(sDot-s_sDot_sDDot_des(:,2));
    ...
    S_tauC    = diag([0, 0, 0, 1, 1, 1]);
    S_tauLeft = [S_tauC,zeros(6,6),zeros(6,ROBOT_DOF)];
    S_tauRight = [zeros(6,6), S_tauC, zeros(6,ROBOT_DOF)];
%     S_fLeft   = [zeros(6,6),zeros(6,6),zeros(6,11),S_fz,zeros(6,6)];
%     S_fRight  = [zeros(6,6),zeros(6,6),zeros(6,11),zeros(6,6),S_fz];
    
    % logic for adding to the cost function the forces minimization task
    if (sum(feetInContact) > 1.5)
        
        if (sum(previousContactState) < 1.5) 
            initialLeftIsFixed   = leftIsFixed;
            previousContactState = feetInContact;
        end
    else
        initialLeftIsFixed   = leftIsFixed;
...
%     alphaR = 0;
%     
%     if (sum(feetInContact) > 1.5) && (leftIsFixed ~= initialLeftIsFixed)
%         
%         if leftIsFixed
...
%     end
    ...
    JDotNuLFoot       = JDot_nu(1:6, :);
    JDotNuRFoot       = JDot_nu(7:12, :);
    JDotNuCoM         = JDot_nu(13:15, :);
    JDotNuRotTask     = JDot_nu(16:18, :);   
    
    % Multiplier of u in the equality constraint equation and bias terms
    JLFoot_invM_B         = JLFoot*invM*B;
    JRFoot_invM_B         = JRFoot*invM*B;
    JLFoot_invM_h         = JLFoot*invM*h;
    JRFoot_invM_h         = JRFoot*invM*h;
    JCoM_invM_B          = JCoM*invM*B;
    JCoM_invM_h          = JCoM*invM*h;
    JRotTask_invM_B      = JRotTask*invM*B;
    JRotTask_invM_h      = JRotTask*invM*h;
    
    acc_LFoot_star        = acc_task_star(1:6);
    acc_RFoot_star        = acc_task_star(7:12);
    acc_Com_star         = acc_task_star(13:15);
    acc_rotTask_star     = acc_task_star(16:18);
    ...
    MBar    = Mj -Mjb/Mb*Mbj;
    hBar    = h(7:end) -Mjb/Mb*h(1:6);
    JBar_t  = transpose(J_c(:,7:end)) -Mjb/Mb*transpose(J_c(:,1:6));
    B_joint = [JBar_t eye(ROBOT_DOF)];
    
    sDDot_star =  MBar*s_sDot_sDDot_des(:,3) ...
                 -diag(impedances)*(s-s_sDot_sDDot_des(:,1)) ....
                 -diag(dampings)*(sDot-s_sDot_sDDot_des(:,2));
    
    % Hessian matrix and gradient for QP solver
    Hessian   =  transpose(B_joint)*B_joint + Sat.weight_tau*transpose(S_tau)*S_tau + ...
                 Sat.weight_fLeft*alphaR*transpose(S_fLeft)*S_fLeft + Sat.weight_fRight*alphaL*transpose(S_fRight)*S_fRight ...
                 +Sat.weight_tauC*transpose(S_tauLeft)*S_tauLeft + +Sat.weight_tauC*transpose(S_tauRight)*S_tauRight;              
             
    gradient  = -transpose(B_joint)*(hBar +sDDot_star);               
            
            
    if rotTaskInCost %Config.rotationalTaskInCost
       Hessian = Hessian + Sat.weight_rotationalTask*transpose(JRotTask_invM_B)*JRotTask_invM_B;
       gradient = gradient -Sat.weight_rotationalTask*transpose(JRotTask_invM_B)*(acc_rotTask_star - JDotNuRotTask + JRotTask_invM_h); 
    end
    
    if Config.feetInCostWhenSwing
        if feetInContact(1) < Sat.toll_feetInContact
            Hessian = Hessian + Sat.weight_Feet*transpose(JLFoot_invM_B)*JLFoot_invM_B;
            gradient = gradient -Sat.weight_Feet*transpose(JLFoot_invM_B)*(acc_LFoot_star - JDotNuLFoot + JLFoot_invM_h);
        end
        
        if feetInContact(2) < Sat.toll_feetInContact
            Hessian = Hessian + Sat.weight_Feet*transpose(JRFoot_invM_B)*JRFoot_invM_B;
            gradient = gradient -Sat.weight_Feet*transpose(JRFoot_invM_B)*(acc_RFoot_star - JDotNuRFoot + JRFoot_invM_h);
        end
    end
    
    % Equality constraints
    
    feetConstraintMatrix    = [JLFoot_invM_B;
                               JRFoot_invM_B];
    feetBiasVectorConstraint = [acc_LFoot_star - JDotNuLFoot + JLFoot_invM_h;
                                acc_RFoot_star - JDotNuRFoot + JRFoot_invM_h];                                
    
    comConstraintMatrix_equality     =  JCoM_invM_B;
    comBiasVectorConstraint_equality = acc_Com_star -JDotNuCoM +JCoM_invM_h;
    
    rotTaskConstraintMatrix_equality     = JRotTask_invM_B;
    rotTaskBiasVectorConstraint_equality = acc_rotTask_star - JDotNuRotTask + JRotTask_invM_h;
    
    ConstraintMatrix_equality     = [feetConstraintMatrix; comConstraintMatrix_equality];
    biasVectorConstraint_equality = [feetBiasVectorConstraint; comBiasVectorConstraint_equality];
    
    if ~rotTaskInCost %~Config.rotationalTaskInCost
        ConstraintMatrix_equality = [ConstraintMatrix_equality; rotTaskConstraintMatrix_equality];
        biasVectorConstraint_equality = [biasVectorConstraint_equality; rotTaskBiasVectorConstraint_equality];
    end
