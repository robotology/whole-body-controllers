...
Sat.weight_tau    = 5;
Sat.weight_fRight = 0.001;
Sat.weight_fLeft  = 0.001;
Sat.weight_rotationalTask = 0.05;
...
Config.smoothingTime_CoM    = [0.5;0.5;0.5];
...
Config.LFoot_Kp_smoothing        = [10;10;10];
Config.RFoot_Kp_smoothing        = [10;10;10];
...
Config.threshold_contact_twoFeet = 30;  % [N]
Config.threshold_contact_on      = 180; % [N]
Config.threshold_contact_off     = 100; % [N]

%% Gains matrices

% CoM position and velocity gains
Gains.Kp_CoM = [60, 70, 50; ...  % state = 1 two feet balancing
                60, 70, 50; ...  % state = 2 left foot balancing
                60, 70, 50];   % state = 3 right foot balancing
                
Gains.Kd_CoM = 2*sqrt(Gains.Kp_CoM)/10;

% Feet position and velocity gains
Gains.Kp_LFoot = [75, 75, 75, 500, 500, 500; ...   % state = 1 two feet balancing
                  75, 75, 75, 500, 500, 500; ...   % state = 2 left foot balancing
                  75, 75, 75, 500, 500, 500];      % state = 3 right foot balancing
              
Gains.Kd_LFoot = 2*sqrt(Gains.Kp_LFoot)/20;

Gains.Kp_RFoot = [75, 75, 85, 500, 500, 500; ...    % state = 1 two feet balancing
                  75, 75, 85, 500, 500, 500; ...    % state = 2 left foot balancing
                  75, 75, 85, 500, 500, 500];       % state = 3 right foot balancing
