function  [H, g, A, ibA, ubA, LB, UB] = MomentumVelocityControl(J_L, J_R, J_CMM, x_com, x_com_des, L_d, int_L_angular, ...
    ConstraintsMatrix, bVectorConstraints,s_k,s_des_k, s_dot_des_k_1,M,h,nu_k,w_H_r_sole,w_H_l_sole,deltaT, Gains,Weigth, Sat)
...
[L_star, s_dot_k_1_star] = ComputeReferences(x_com, x_com_des, L_d, int_L_angular, s_dot_des_k_1, s_des_k, s_k, Gains); 

% Computing Tasks Matrices  
% Postural 
[A_p, a_p] = ComputePosturalTask(s_dot_k_1_star, NDOF); 
% Compute Hessian and Gradient
[H_p, g_p] = ComputeHessianAndGradient(A_p,a_p, Weigth.PosturalTask); 

% Torque Minimization 
[A_tau, a_tau] = ComputeTorqueMinimizationTask(B, M, deltaT, Jc, h, nu_k); 
% Compute Hessian and Gradient
[H_tau, g_tau] = ComputeHessianAndGradient(A_tau,a_tau, Weigth.MinimizationTorques);


% Compute Constraints 
% Equality 
[C_eq, b_eq] = ComputeEqualityConstraint(Jc, J_CMM, L_star); 
% Inequality 
[C_in, b_in] = ComputeInequalityConstraint(ConstraintsMatrixBothFeet, bVectorConstraintsBothFeet, NDOF); 
