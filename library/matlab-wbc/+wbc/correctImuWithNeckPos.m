function imu_H_imuAssumingNeckToZero = correctImuWithNeckPos(neckJoints)
    ...
    H_34 = wbc.evalDHMatrix( 9.5*1e-3,          0,  pi/2, +pi/2);
    H_45 = wbc.evalDHMatrix(        0,          0, -pi/2, -pi/2);
    H_56 = wbc.evalDHMatrix(18.5*1e-3, 110.8*1e-3, -pi/2, +pi/2); 
    H_61 = wbc.evalDHMatrix(        0,   6.6*1e-3,  pi/2,     0); 

    imuAssumingNeckToZero_H_neckBase = H_34*H_45*H_56*H_61;

    % Compute the imu_H_neckBase transform 
    H_34 = wbc.evalDHMatrix( 9.5*1e-3,          0,  pi/2, neckJoints(1) +pi/2);
    H_45 = wbc.evalDHMatrix(        0,          0, -pi/2, neckJoints(2) -pi/2);
    H_56 = wbc.evalDHMatrix(18.5*1e-3, 110.8*1e-3, -pi/2, neckJoints(3) +pi/2); 
    H_61 = wbc.evalDHMatrix(        0,   6.6*1e-3,  pi/2,                   0); 
