function references_CoM = referenceGeneratorCoM_jerkControl(pos_CoM_0, t, Config)

    % REFERENCEGENERATORCOM_JERKCONTROL computes a CoM reference trajectory. 
    %                                   Default trajectory is a sine function.
    %
    % USAGE: please note that this function has been designed for being inserted 
    %        in a Simulink model.
    %
    % FORMAT: references_CoM = referenceGeneratorCoM_jerkControl(pos_CoM_0, t, Config)
    %
    % INPUT:  - pos_CoM_0 = [3 * 1] initial CoM position
    %         - t = simulation time
    %         - Config = user defined configuration
    %
    % OUTPUT: - references_CoM = [12 * 1] desired CoM position, velocity, 
    ...
    if Config.amplitudeOfOscillation ~= 0
        ...
        jerk_CoM_des  =           -Amplitude*(2*pi*frequency)^3*cos(2*pi*frequency*t)*Config.directionOfOscillation;
    end
    
    references_CoM    = [pos_CoM_des; vel_CoM_des; acc_CoM_des; jerk_CoM_des];
end